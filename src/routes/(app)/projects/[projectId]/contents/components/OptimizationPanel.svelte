<script lang="ts">
	...
	const metrics = $derived(
		getStructureMetrics(content.serpmanticsGuide, analysis),
	);
	...
	async function retry() {
		refreshing = true;
		try {
			const updated = await retryOptimizationGuide({
				id: content.id,
				projectId: content.projectId,
			});
			onContentUpdated(updated);
			...
			toast.error(
				error instanceof Error ? error.message : "Nouveau guide impossible.",
				{ richColors: true },
			);
		} finally {
			refreshing = false;
	...
	function expressionState(expression: SerpmanticsExpression) {
		if (expression.from == null || expression.to == null)
			return occurrence(expression) > 0 ? "remove" : "valid";
		return getOptimizationState(occurrence(expression), {
	...
	function resultUrl(result: Record<string, unknown>) {
		return String(
			result.url ?? result.link ?? result.href ?? "Résultat non pris en compte",
		);
	}
	...
	function resultScore(result: Record<string, unknown>) {
		const value =
			result.score ??
			(result.contentAnalysis as Record<string, unknown> | undefined)?.score;
...
{#if content.serpmanticsStatus === "pending"}
	<section class="PanelCard LoadingState">
		...
		<h2>Préparation de l’analyse SEO</h2>
		<p>L’analyse est en cours. Cette page s’actualisera automatiquement.</p>
	</section>
{:else if content.serpmanticsStatus === "failed" || !content.serpmanticsGuideId}
	<section class="PanelCard LoadingState ErrorState">
		<h2>Analyse SEO indisponible</h2>
		<p>{content.serpmanticsError || "L’analyse n’a pas pu être préparée."}</p>
		<button class="btn btn-primary" disabled={refreshing} onclick={retry}
			>Réessayer</button
		>
	</section>
{:else}
	<section class="PanelCard ScoreCard">
			...
			<p>
				<strong
					>{score >= 50
						...
						: "Votre contenu peut encore gagner en couverture sémantique."}</strong
				>
				{score >= 50
					? " Continuez à insérer les expressions ci-dessous pour dépasser 75% de vos concurrents."
					: " Utilisez les recommandations ci-dessous pour progresser."}
			</p>
		</div>
		<div class="ScoreGauge">
			...
			<div class="ScoreScale">
				<span
					class="ScoreMarker"
					style:left={`${Math.min(100, (score / 120) * 100)}%`}
				></span>
			</div>
		</div>
		<div class="Legend">
			<span class="LegendSwatch valid"></span>Validé
			<span class="LegendSwatch add"></span>À ajouter
			<span class="LegendSwatch remove"></span>À enlever
		</div>
		<div class="MetricsGrid">
			{#each metrics as metric (metric.label)}
				<div class="Metric state-{metric.state}">
					<span class="MetricLabel" class:compact={metric.label.length >= 8}
						>{metric.label}</span
					>
					<div class="MetricValue">
						<strong>{metric.value}</strong>
						<span class="MetricState" aria-hidden="true"
							>{metric.state === "valid"
								? "✓"
								: metric.state === "add"
									? "↗"
					...
					<small
						>{metric.range
							? `${metric.range.from} à ${metric.range.to}`
							: "—"}</small
					>
				</div>
			{/each}
	...
	<section class="PanelCard ExpressionCard">
		<header class="PanelTitleRow">
			<div>
				<h2>Expressions à ajouter dans votre contenu</h2>
		...
		<div class="ExpressionCloud">
			{#each addExpressions as expression (expression.expression)}
				<span class="Expression state-{expressionState(expression)}">
					...
					<span class="ExpressionMeta"
						><strong>{occurrence(expression)}</strong
						>{#if expression.from != null}<small
								>· {expression.from} à {expression.to}</small
							>{/if}</span
					>
				</span>
			{/each}
			{#if addExpressions.length === 0}<p class="Muted">
					Aucune expression supplémentaire.
				</p>{/if}
		</div>
	</section>
	...
	<section class="PanelCard ExpressionCard">
		<header class="PanelTitleRow">
			<h2>Expressions à éviter</h2>
		</header>
		<div class="ExpressionCloud">
			{#each avoidExpressions as expression (expression.expression)}
				<span class="Expression avoid"
					>{expression.expression}{#if occurrence(expression)}
						<strong>{occurrence(expression)}</strong>{/if}</span
				>
			{/each}
			{#if avoidExpressions.length === 0}<p class="Muted">
					Aucune expression à éviter.
				</p>{/if}
		</div>
	</section>
					...
					<div class="ResultContent">
						{#if resultUrl(result).startsWith("http")}
							<a
								...
								target="_blank"
								rel="noopener noreferrer">{resultUrl(result)}</a
							>
						{:else}
							<strong>{resultUrl(result)}</strong>
						{/if}
						{#if resultScore(result) != null}<OptimizationScore
								score={resultScore(result)}
							/>{/if}
					</div>
				</div>
			{/each}
			{#if (content.serpmanticsGuide?.topSERPResultsDetails?.length ?? 0) === 0}<p
					class="Muted"
				>
					Les détails des résultats ne sont pas disponibles.
				</p>{/if}
		</div>
	</section>
{/if}
...
<style>
	...
	.MetricsGrid {
		...
		grid-template-rows: repeat(2, 6.1rem);
	...
	.Metric.state-valid strong {
		color: #08751f;
	}
	.Metric.state-valid .MetricState {
		color: #08751f;
		background: #b8fac4;
	}
	.Metric.state-add strong {
		color: #d56b00;
	}
	.Metric.state-add .MetricState {
		color: #8a5700;
		background: #ffe6a0;
	}
	.Metric.state-remove strong {
		color: #a90e15;
	}
	.Metric.state-remove .MetricState {
		color: #a90e15;
		background: #ffc4c7;
	}
	.Metric small {
