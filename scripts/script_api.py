...
DEVICE = "desktop"
DEPTH = 50 # Nombre de résultats Google (max. 10 maintenant)
...
print(f"Tâches créées ({len(task_id_to_keyword)} au total). Attente initiale de 90s pour génération SERP...")
        ...
        for resultTaskInfo in task.get("result", []):
            ...
            if id_ in task_id_to_keyword and id_ not in collected:
                ...
                if resultTaskInfo.get('endpoint_regular'):
                    ...
                    res = client.get(f"/v3/serp/google/organic/task_get/regular/{reg_id}")
                else:
                ...
                if res.get("tasks") and res["tasks"][0].get("result"):
                    ...
                    for item in items:
                        ...
                        all_results.append({
                            ...
                            "serp_features": ", ".join([f.get("feature") for f in features]) if features else "",
...
if all_results:
    pd.DataFrame(all_results).to_csv("serps_resultats.csv", index=False, encoding="utf-8")
