...
# =========================
TAUX_SIMILARITE = 0.60  # Seuil de similarité (0.60 = 60%)
NB_URLS_ANALYSE = 10  # Top N URLs par mot-clé (type "organic")
TYPE_RESULTAT = "organic"  # Type de résultat à considérer
INPUT_FILE = "serps_resultats"  # accepte "serps_resultats" ou "serps_resultats.csv"
OUTPUT_FILE = "clusters_serp.csv"

# Normalisation d'URL
EXCLURE_PARAMS_FIXES = {"gclid", "fbclid", "srsltid", "mc_eid", "igshid"}
...
def build_keyword_urlsets(df: pd.DataFrame, topn: int, result_type: str):
    ...
    volumes = (
        df.groupby(df["keyword"].astype(str), sort=False)["volume"].max().to_dict()
    )
...
def make_export_df(clusters, volumes: dict) -> pd.DataFrame:
        ...
        rows.append(
            {
                ...
                "Volume de recherche du mot-clé principal": int(main_vol)
                if main_vol.is_integer()
                else main_vol,
                "Volume de recherche total": int(total_vol)
    ...
    out = out.sort_values(
        by=["Volume de recherche total", "Mot-clé principal"],
...
def main():
    ...
    urlsets, volumes = build_keyword_urlsets(
        df, topn=cfg["topn"], result_type=cfg["result_type"]
    )
